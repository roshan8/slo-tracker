...
function App() {
  ...
  const [errBudgetOverTime, setErrBudgetOverTime] = useState<
    errBudgetOverTimeType[]
  >([]);
  const { TabPane } = Tabs;
  ...
  const SLOFormLayout = () => {
    const onFinish = (values: any) => {
      const createSLOApi = async () => {
        try {
          const sloCreationReq = await fetch(`${API_URL}/api/v1/slo/Unnamed`, {
            method: 'PATCH',
            body: JSON.stringify({
          ...
          openNotificationWithIcon(
            'error',
            'Unable to update the target SLO :('
          );
          console.log(err);
        }
        setIsSLODrawerVisible(false);
    ...
    },
    {
      ...
      render: (e, record) => (
        <Switch
          ...
          defaultChecked={e.mark_false_positive}
        />
      ),
    },
  ];
      ...
      while (i < incidentList.length) {
        ...
        for (j = 0; j < incidentSummaryArr.length; j++) {
          ...
          if (incidentList[i]['sli_name'] === incidentSummaryArr[j]['id']) {
            incidentSummaryArr[j]['value'] +=
              incidentList[i]['err_budget_spent'];
            sliNameFound = 1;
          }
        }
        ...
        for (j = 0; j < errBudgetArr.length; j++) {
          ...
          if (
            incidentList[i]['created_at'].split('T', 1)[0] ===
            errBudgetArr[j]['x']
          ) {
  ...
  const getSLOApiCall = async () => {
    try {
      const SLOListResponse = await fetch(`${API_URL}/api/v1/slo/Unnamed`)
        ...
        .then((response) => {
          ...
          CalculateErrBudget(
            response.data.target_slo,
            response.data.remaining_err_budget
          );
        })
        .catch((err) => {
          console.log(err);
